...
public class HangfireInit
{
    public static void InitRecurrentJobs(IConfiguration configuration)
    {
        ...
        RecurringJob.AddOrUpdate<CompetitionConductor>("Stop competition", x => x.StopAsync(), "1 17 * * *");
        RecurringJob.AddOrUpdate<CompetitionConductor>("Season results", x => x.SeasonResultsAsync(), "2 17 * * *");
        RecurringJob.AddOrUpdate<CompetitionConductor>("Start new competition", x => x.StartNewAsync(), "3 17 * * *");
